        ...
        <div>
            ...
            >
                ...
                >
                    <template #label>
                        <span>
                            <FontAwesomeIcon icon="key" /> Key Tag
                        </span>
                    </template>
                    <el-input-number
                ...
                >
                    <template #label>
                        <span>
                            <FontAwesomeIcon icon="cog" /> 签名算法
                        </span>
                    </template>
                    <el-select
                        v-model="model.data.algorithm"
                        placeholder="选择算法"
                    >
                ...
                <el-form-item
                    v-if="isDS"
                    prop="data.digestType"
                >
                    <template #label>
                        <span>
                            <FontAwesomeIcon icon="fingerprint" /> 摘要算法
                        </span>
                    </template>
                    <el-select
                        v-model="model.data.digestType"
                        placeholder="选择摘要算法"
                    >
                ...
                >
                    <template #label>
                        <span>
                            <FontAwesomeIcon icon="font" /> 摘要
                        </span>
                    </template>
                    <el-input
                        v-model="model.data.digest"
                        placeholder="十六进制摘要值"
                    />
                </el-form-item>
                ...
                <el-form-item v-if="isDNSKEY" prop="data.flags">
                    <template #label><span><FontAwesomeIcon icon="flag" /> Flags</span></template>
                ...
                <el-form-item v-if="isDNSKEY">
                    <template #label><span><FontAwesomeIcon icon="cog" /> Protocol</span></template>
                    <el-input :model-value="3" disabled />
                </el-form-item>
                <el-form-item v-if="isDNSKEY" prop="data.algorithm">
                    <template #label><span><FontAwesomeIcon icon="cog" /> 签名算法</span></template>
                ...
                <el-form-item v-if="isDNSKEY" prop="data.publicKey">
                    <template #label><span><FontAwesomeIcon icon="key" /> 公钥</span></template>
                    <el-input v-model="model.data.publicKey" type="textarea" :rows="2" placeholder="Base64 编码的公钥" />
                </el-form-item>

                <!-- CERT fields -->
                <el-form-item v-if="isCERT" prop="data.type">
                    <template #label><span><FontAwesomeIcon icon="file" /> Type</span></template>
                ...
                <el-form-item v-if="isCERT" prop="data.keyTag">
                    <template #label><span><FontAwesomeIcon icon="key" /> Key Tag</span></template>
                ...
                <el-form-item v-if="isCERT" prop="data.certificate">
                    <template #label><span><FontAwesomeIcon icon="file" /> 证书</span></template>
                    <el-input v-model="model.data.certificate" type="textarea" :rows="2" placeholder="证书内容" />
                </el-form-item>

                <!-- SSHFP fields -->
                <el-form-item v-if="isSSHFP" prop="data.algorithm">
                    <template #label><span><FontAwesomeIcon icon="cog" /> 算法</span></template>
                    <el-select v-model="model.data.algorithm" placeholder="选择算法">
                ...
                <el-form-item v-if="isSSHFP" prop="data.type">
                    <template #label><span><FontAwesomeIcon icon="file" /> 类型</span></template>
                    <el-select v-model="model.data.type" placeholder="选择">
                        <el-option label="1 - SHA-1 (不推荐)" :value="1" />
                ...
                <el-form-item v-if="isSSHFP" prop="data.fingerprint">
                    <template #label><span><FontAwesomeIcon icon="fingerprint" /> 指纹</span></template>
                    <el-input v-model="model.data.fingerprint" placeholder="十六进制指纹" />
                </el-form-item>

                <!-- TLSA / SMIMEA fields -->
                <el-form-item v-if="isTLSAorSMIMEA" prop="data.usage">
                    <template #label><span><FontAwesomeIcon icon="flag" /> Usage</span></template>
                ...
                <el-form-item v-if="isTLSAorSMIMEA" prop="data.selector">
                    <template #label><span><FontAwesomeIcon icon="file" /> Selector</span></template>
                ...
                <el-form-item v-if="isTLSAorSMIMEA" prop="data.matchingType">
                    <template #label><span><FontAwesomeIcon icon="fingerprint" /> Matching Type</span></template>
                ...
                <el-form-item v-if="isTLSAorSMIMEA" prop="data.certificate">
                    <template #label><span><FontAwesomeIcon icon="file" /> 证书/公钥</span></template>
                    <el-input v-model="model.data.certificate" type="textarea" :rows="2" placeholder="证书或公钥内容" />
                </el-form-item>

                <!-- NAPTR fields -->
                <el-form-item v-if="isNAPTR" prop="data.order">
                    <template #label><span><FontAwesomeIcon icon="sort-numeric-up" /> Order</span></template>
                    <el-input-number v-model="model.data.order" :min="0" :max="65535" />
                </el-form-item>
                <el-form-item v-if="isNAPTR" prop="data.preference">
                    <template #label><span><FontAwesomeIcon icon="sort-numeric-up" /> Preference</span></template>
                    <el-input-number v-model="model.data.preference" :min="0" :max="65535" />
                </el-form-item>
                <el-form-item v-if="isNAPTR" prop="data.flags">
                    <template #label><span><FontAwesomeIcon icon="flag" /> Flags</span></template>
                    <el-select v-model="model.data.flags" allow-create filterable placeholder="选择或输入">
                ...
                <el-form-item v-if="isNAPTR" prop="data.service">
                    <template #label><span><FontAwesomeIcon icon="font" /> Service</span></template>
                    <el-input v-model="model.data.service" placeholder="例如: sip+E2U" />
                </el-form-item>
                <el-form-item v-if="isNAPTR" prop="data.regex">
                    <template #label><span><FontAwesomeIcon icon="font" /> Regexp</span></template>
                    <el-input v-model="model.data.regex" placeholder='例如: !^.*$!sip:info@example.com!' />
                </el-form-item>
                <el-form-item v-if="isNAPTR" prop="data.replacement">
                    <template #label><span><FontAwesomeIcon icon="font" /> Replacement</span></template>
                    <el-input v-model="model.data.replacement" placeholder="例如: _sip._udp.example.com." />
                </el-form-item>

                <!-- LOC fields -->
                <el-form-item v-if="isLOC" prop="data.latDegrees">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 纬度度</span></template>
                    <el-input-number v-model="model.data.latDegrees" :min="0" :max="90" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.latMinutes">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 纬度分</span></template>
                    <el-input-number v-model="model.data.latMinutes" :min="0" :max="59" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.latSeconds">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 纬度秒</span></template>
                    <el-input-number v-model="model.data.latSeconds" :min="0" :max="59.999" :precision="3" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.latDirection">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 纬度方向</span></template>
                ...
                <el-form-item v-if="isLOC" prop="data.longDegrees">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 经度度</span></template>
                    <el-input-number v-model="model.data.longDegrees" :min="0" :max="180" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.longMinutes">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 经度分</span></template>
                    <el-input-number v-model="model.data.longMinutes" :min="0" :max="59" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.longSeconds">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 经度秒</span></template>
                    <el-input-number v-model="model.data.longSeconds" :min="0" :max="59.999" :precision="3" />
                </el-form-item>
                <el-form-item v-if="isLOC" prop="data.longDirection">
                    <template #label><span><FontAwesomeIcon icon="globe-asia" /> 经度方向</span></template>
...
const requirePriority = computed(() => {
    return model.value.type === 'MX' || model.value.type === 'SRV' || model.value.type === 'URI' || isHTTPSSVCB.value
})
...
const hasStructuredForm = computed(() => {
    return isSRV.value || isCAA.value || isHTTPSSVCB.value || isDS.value
        || isDNSKEY.value || isCERT.value || isSSHFP.value || isTLSAorSMIMEA.value
        || isNAPTR.value || isLOC.value || isURI.value
})
...
watch(() => model.value.type, (newType) => {
    ...
    if (newType === 'DNSKEY' && model.value.data.flags !== 256 && model.value.data.flags !== 257) {
